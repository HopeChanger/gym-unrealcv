...
class Tracking(Navigation):
    ...
    def random_texture(self, backgrounds, img_dirs, num=5):
        ...
        for id in sample_index:
            ...
            self.set_simple_texture(target, img_dir)
            time.sleep(0.03)

    def random_player_texture(self, player, img_dirs, num=3):
        img_dir = img_dirs[np.random.randint(0, len(img_dirs))]
        self.set_simple_texture(player, img_dir)
        time.sleep(0.03)
    ...
    def auto_move(self, target, auto=True, time_step=5, min_speed=100, max_speed=300):
        cmd = 'vbp {target} auto_move {auto} {time} {min} {max}'
        res = None
        while res is None:
            res = self.client.request(cmd.format(target=target, auto=auto, time=time_step, min=min_speed, max=max_speed))
        return auto
    ...
    def random_obstacles(self, objects, img_dirs, num, area, start_area, obstacle_scales=None):
        ...
        for k, id in enumerate(sample_index):
            ...
            # texture
            img_dir = img_dirs[np.random.randint(0, len(img_dirs))]
            self.set_simple_texture(obstacle, img_dir)
            # scale

...
class UnrealCv(object):
    ...
    def set_location(self,cam_id, loc):  # loc=[x,y,z]
        ...
        out = self.client.request(cmd.format(cam_id=cam_id, x=loc[0], y=loc[1], z=loc[2]))
    ...
    def set_rotation(self,cam_id, rot):  # rot = [roll, yaw, pitch]
        ...
        out = self.client.request(cmd.format(cam_id=cam_id, roll=rot[0], yaw=rot[1], pitch=rot[2]))
    ...
    def pause(self):
        cmd = 'vset /action/game/pause'
        return self.client.request(cmd)

    def resume(self, run_time=0):
        cmd = 'vset /action/game/resume'
    ...
    def is_paused(self):
        ...
        pause = None
        while pause is None:
            pause = self.client.request('vget /action/game/is_paused')
        return bool(pause)
